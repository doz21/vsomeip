...
ttl_t serviceinfo::get_ttl() const {
  boost::lock_guard<boost::mutex> its_lock(ttl_mutex_);
...
std::shared_ptr<endpoint> serviceinfo::get_endpoint(bool _reliable) const {
  boost::lock_guard<boost::mutex> its_lock(endpoint_mutex_);
...
void serviceinfo::add_client(client_t _client) {
  boost::lock_guard<boost::mutex> its_lock(requesters_mutex_);
